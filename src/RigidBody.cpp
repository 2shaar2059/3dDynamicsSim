...
RigidBody::RigidBody(double mass, Matrix3d InertiaTensor, Quaternion<double> rotation_initial, Vector3d w_b_initial,
    ...
    x.segment(0,
              3) = P_g_initial; //using a block operation to embed the 3 components of the initial-global-linear-position-vector into indexes 0,1,2 of the overall state vector, x

    x.segment(3,
              3) = V_g_initial; //using a block operation to embed the 3 components of the initial-global-linear-velocity-vector into indexes 3,4,5 of the overall state vector, x

    x.segment(6,
              4) = rotation_initial.coeffs(); //using a block operation to embed the 4 components of the initial-orientation-quaternion into indexes 6,7,8,9 of the overall state vector, x

    Vector3d H_b_initial = InertiaTensor * w_b_initial;
    x.segment(10,
              3) = H_b_initial; //using a block operation to embed the 3 components of the initial-body-angular-momentum-vector into indexes 10,11,12 of the overall state vector, x
...
Quaternion<double> RigidBody::getRotationGtoB() {
    return Quaternion<double>(x(9), x(6), x(7),
                              x(8)); //returning the quaternion that is represented by the 4 quaternion coefficents that are embedded in indexes 6,7,8,9 of the state vector, x
}
...
void RigidBody::applyMoment(Vector3d moment) {
    u.segment(3,
              3) += moment; //using a block operation to add the 3 components of the applied-body-moment-vector to indexes 3,4,5 of the overall input vector, u
}

void RigidBody::applyForce(Vector3d force, Vector3d pointOfApplication) {
    u.segment(0,
              3) += force; //using a block operation to add the 3 components of the applied-body-force-vector to indexes 0,1,2 of the overall input vector, u
...
Vector13d RigidBody::f(Vector13d x, Vector6d u) {
    ...
    xdot.segment(0, 3) = x.segment(3,
                                   3); //using a block operation to copy the 3 components of the state-vector at indexes 3,4,5 (which represent global-linear-velocity) into indexes 0,1,2 of the derivative-of-the-overall-state-vector
    Vector3d FAppNet_b = u.segment(0,
                                   3); //using a block operation to copy the 3 components of the input-vector at indexes 0,1,2 (which represent net-body-applied-force) into the net-body-applied-force-vector, FAppNet_b
    Vector3d Anet_g = (getRotationBtoG() * FAppNet_b) / m +
                      g; //finding the net-global-linear-acceleration after taking into account the applied forces and the effect of gravity
    xdot.segment(3,
                 3) = Anet_g; //using a block operation to embed the 3 components of the net-global-linear-acceleration-vector (derivative of global-linear-velocity) into indexes 3,4,5 of the derivative-of-the-overall-state-vector

    Vector3d H_b = x.segment(10,
                             3); //using a block operation to copy the 3 components of the state-vector at indexes 10,11,12 (which represent body-angular-momentum) into the body-angular-momentum-vector, H_b
    Vector3d w_b = InertiaTensorInverse * H_b;
    Quaternion<double> w_b_quaternion_form = Quaternion<double>(0, w_b(0), w_b(1),
                                                                w_b(2)); //converting the 3-component vector-representation of the body-angular-velocity into an equivalent quaternion form
    Quaternion<double> qdot = Quaternion<double>(0.5 * (getRotationGtoB() *
                                                        w_b_quaternion_form).coeffs()); //calculating the derivative of the orientation-qauternion (formula 14 from pg.7 of https://arxiv.org/pdf/0811.2889.pdf)
    xdot.segment(6,
                 4) = qdot.coeffs(); //using a block operation to embed the 4 components of the derivative-of-the-orientation-quaternion, qdot, into indexes 6,7,8,9 of the derivative-of-the-overall-state-vector

    xdot.segment(10, 3) = u.segment(3,
                                    3); //using a block operation to copy the 3 components of the input-vector at indexes 3,4,5 (which represent net-body-applied-torque) into indexes 10,11,12 of the derivative-of-the-overall-state-vector
...
void RigidBody::update(double dt) {
    ...
    Vector3d V_g = x.segment(3,
                             3); //using a block operation to copy the 3 components of the state-vector at indexes 3,4,5 (which represent global-linear-velocity) into the global-linear-velocity-vector, V_g
    velX_global_arr.push_back(V_g(0));
    velY_global_arr.push_back(V_g(1));
    velZ_global_arr.push_back(V_g(2));

    Vector3d P_g = x.segment(0,
                             3); //using a block operation to copy the 3 components of the state-vector at indexes 0,1,2 (which represent global-linear-position) into the global-linear-position-vector, P_g
    ...
    x.segment(6,
              4) /= q_length; //using a block operation to renormalize the 4 components of the state vector at indexes 6,7,8,9 which makeup the Rigid Body's orientation-quaternion
