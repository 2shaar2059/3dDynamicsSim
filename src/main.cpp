...
int main()
{
	...
	double maxTime = 1;
	double currTime = 0;
	double dt = 0.000001; //timestep between state updates
	...
	Vector3d ang_vel_inital;	ang_vel_inital << 0,	999,	rad(0); //thetaXdot, thetaYdot, thetaZdot 	[rad/s, rad/s, rad/s]
	...
	while (currTime <= maxTime) {
		...
		rigidBody1.applyForce(rigidBody1.getRotationGtoB()*Vector3d(0, 0, -1), Vector3d(0, 1, 0));
